    ...
    while(1)
    {
        ...
        switch(ch)
        {
            ...
            case 6:
            {
                if(t.reduceheapify())
                {
                    printf("\nReduced the given elements suceesfully");
                }
                else
                {
...
int heap::insert(int num)
{
    ...
    else
    {
        ...
        {
            n++;
            arr=(int*)realloc(arr,sizeof(arr) + sizeof(int)*pow(2,n));
            cur++;
            arr[cur]=num;
            insertheap(cur);
            return 1;
...
int heap::deletion() 
{
    ...
    else 
    {
        ...
        while (1) 
        {
            ...
            int maxIndex = parent; // Assume parent is the largest initially

            // Find the largest among parent, left child, and right child
            if (leftChild <= cur && arr[leftChild] > arr[maxIndex]) 
            {
                maxIndex = leftChild;
            }
            if (rightChild <= cur && arr[rightChild] > arr[maxIndex]) 
            {
                maxIndex = rightChild;
            }
            ...
            { // If parent is not the largest
                ...
                parent = maxIndex; // Move down the tree
            } 
            else 
            {
...
void heap::inserts(int num)
{
    ...
    else
    {
        ...
        {
            ns++;
            arrs=(int*)realloc(arrs,sizeof(arrs) + sizeof(int)*pow(2,ns));
            curs++;
            arrs[curs]=num;
        }
        else
...
void heap::heapify(int current)
{
    ...
    while(1)
    {
        ...
        rchild=2*parent+2;
        maxindex=parent;
        if(lchild<=cur && arr[lchild]>arr[parent])
        {
            maxindex=lchild;
        }
        if(rchild<=cur && arr[rchild]>arr[parent])
        {
            maxindex=rchild;
        }
        if(maxindex!=parent)
